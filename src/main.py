...
from plot_stat import plot_graphs, save_graphs
import random
import csv
...
def main():
    ...
    # run a batch of test cycles for graphs of varying sizes
    for num_nodes in range(10, 51, 10):
        stats = []
        ...
        for link_prob in [x / 100.0 for x in range(5, 100, 1)]:
            ...
            generate_graph(network, num_nodes, link_prob)
            ...
            for _ in range(0, num_packets):
                src_node = random.choice([n for n in network.nodes])
                dest_node = random.choice([n for n in network.nodes if n is not src_node])
                packet = Packet(
                    "data",
                    src_node.identifier,
            ...
            mutate_graph(network.nodes, link_prob)    
            print()
        ...
        link_probs = [row[0] for row in stats] 
        losses = [row[1] for row in stats]
        delays = [row[2] for row in stats]
        throughputs = [row[3] for row in stats]
        plot_graphs(num_nodes, link_probs, losses, delays, throughputs)
